...
def display(
    prompt, inputStrings, variables, color=None, waitForInputValue="", delay=0.03
):
    # Setting up colors.
        ...
        for inputString in inputStrings:
            ...
            for element in inputElements:
                ...
                if len(element) > 0 and element[0] == ">":
                    ...
                    if element[1] == "P" and not variables["Debug"]:
                        cursor.hide()
                    ...
                    # If we have an action, we call the function with the name specified in the string.
                        ...
                        actionName = element[3:]
                        actionsFunction = getattr(actions, actionName)

                        if actionsFunction is not None:
                            actionsFunction(variables)
                    ...
                    # If we have a read action, we wait for the user to press enter and store the input in the variables dictionary.
                        ...
                        if len(element) > 3:
                            ...
                            sys.stdout.write(color_codes[color])
                            if (
                                readInput.lower() != waitForInputValue.lower()
                                and waitForInputValue != ""
                            ):
                                display(
                                    prompt,
                                    [
                                        "#>C:red#"
                                        + Sentences.labNotUnderstood[
                                            variables["Language"]
                                        ]
                                        + "#>D#"
                                    ],
...
def stageMessage(id_number, json_file_path, language="en"):
    ...
    return (
        info["Prompt"] if "Prompt" in info else "-:-",
        info["Messages"][language],
        info["DefaultColor"],
        waitForInputValue,
        checkTask,
        info["SaveScore"] if "SaveScore" in info else True,
        info["SilentOnSuccess"] if "SilentOnSuccess" in info else False,
    )
...
def clueMessage(checkScript, messageNumber, language="en"):
    with open(main.labAnswersJsonFile, "r") as file:
        data = json.load(file)
...
def checkStage(checkScript, prompt, color, variables, recoveryMode, silent="None"):
    checkFunction = getattr(CheckLabs, checkScript)

    if checkFunction is not None:
        ret = False

        # We do force silent mode for predefinied checks.
        if checkScript in main.forceSilentModeDuringChecks:
            silent = "Full"

        while not ret:
            ret, messageNumber, reenterValue = checkFunction(
                variables, recoveryMode
            )
            ...
            if silent == "Full":
                ...
                errorMessage = (
                    random.choice(Sentences.labKo[variables["Language"]]) + "\n"
                )

                if reenterValue is not None:
                    retryMessage = Sentences.labRetryWithValue[variables["Language"]]
                else:
                    retryMessage = Sentences.labRetry[variables["Language"]]
            ...
            if not ret:
                ...
                if silent == "None":
                    display(
                        prompt,
                        [
                            random.choice(Sentences.labOk[variables["Language"]])
                            + "#>P:3#\n\n"
                        ],
                        variables,
                        color,
...
def updateScoreFile(scoreFolder, trigram, stage, maxStage, variables=None):
    ...
    try:
        ...
        scoreJson = {
            ...
            "lastUpdated": time.strftime("%H:%M:%S", time.localtime()),
            "finishedTime": "",
            "duration": "",
        }
    ...
    if stage >= maxStage:
        ...
        start_seconds = (
            start_struct.tm_hour * 3600 + start_struct.tm_min * 60 + start_struct.tm_sec
        )
        finish_seconds = (
            finish_struct.tm_hour * 3600
            + finish_struct.tm_min * 60
            + finish_struct.tm_sec
        )
    ...
    try:
        ...
        print(
            "Error, unable to write to the score file. Please check the path and permissions."
        )
        sys.exit(4)

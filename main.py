...
if __name__ == "__main__":
    ...
    if "-clean" in sys.argv:
        ...
        escapegameengine.cleanScoreFiles(scoreFolder, maxStages)
        print("Game cleaned")
        sys.exit(0)
    ...
    if "-changeStage" in sys.argv:
        try:
            ...
            print(
                "Invalid stage ID. Please provide a valid number between 1 and",
                maxStage,
            )
            sys.exit(1)
    ...
    for stage in data["stages"]:
        ...
        (
            ...
            saveScore,
            SilentOnSuccess,
        ) = escapegameengine.stageMessage(
            stage["id"], contentJsonFile, variables["Language"]
        )
        # print(f"\n--- Stage {stage['id']} ---")
        ...
        if stage["id"] <= variables["RecoveryUntilStage"] and stage["active"]:
            ...
            if checkScript != "":
                escapegameengine.checkStage(
                    checkScript, prompt, color, variables, True, "Full"
                )

        elif stage["active"]:
            # We display the message because we are not recovering the stage and it is active
            escapegameengine.display(
                prompt, messages, variables, color, waitForInputValue
            )
            ...
            if checkScript != "":
                escapegameengine.checkStage(
                    ...
                    variables,
                    False,
                    "NoSuccess" if SilentOnSuccess else "None",
                )

        # Update the score file
        if saveScore and variables["Trigram"]:
            escapegameengine.updateScoreFile(
                scoreFolder,
                variables["Trigram"].lower(),
                stage["id"],
                maxStage,
                variables,
            )

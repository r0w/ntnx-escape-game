...
def retrieveAuthorizationPolicyId(policyName, variables):
    ...
    client = ntnx_iam_py_client.ApiClient(configuration=sdkConfig)
    authorizationPoliciesApi = ntnx_iam_py_client.AuthorizationPoliciesApi(
        api_client=client
    )

    response = authorizationPoliciesApi.list_authorization_policies(
        _filter="displayName eq '" + str(policyName) + "'"
    )
    myData = response.to_dict()
...
def checkAuthorizationPolicyAssignement(
    authorizationPolicyId, roleId, userId, variables
):
    # Configure the client
    sdkConfig = ntnx_networking_py_client.Configuration()
    sdkConfig.host = variables["PC"]
    ...
    response = authorizationPoliciesApi.get_authorization_policy_by_id(
        extId=authorizationPolicyId
    )
    myData = response.to_dict()
    ...
    user_assigned = any(
        identity["_reserved"]["user"]["uuid"]["anyof"][0] == userId
        for identity in myData["data"]["identities"]
    )
...
def retrieveSubnetID(subnet_name, variables):
    ...
    if myData["data"] is None or len(myData["data"]) != 1:
        return None
...
def hasVMCloudinit(vmuuid, pc, user, password):
    ...
    try:
        if response_data["spec"]["resources"]["guest_customization"] is None:
            return False
    except Exception:
...
def createVMRecoveryPoint(vm_uuid, variables):
    ...
    url = (
        "https://%s:9440/api/dataprotection/v4.0/config/recovery-points"
        % variables["PC"]
    )
    headers = {
    ...
    payload = {"vmRecoveryPoints": [{"vmExtId": vm_uuid}]}
...
def retrieveCatID(key, value, variables):
    ...
    if value is None:
        try:
            api_response = categories_api.list_categories(
                _page=page, _limit=limit, _filter="key eq '" + key + "'"
            )
        ...
        try:
            api_response = categories_api.list_categories(
                _page=page,
                _limit=limit,
                _filter="key eq '" + key + "' and value eq '" + value + "'",
            )
            myData = api_response.to_dict()
...
def retrieveStoragePolicyID(policy_name, variables):
    ...
    payload = {"kind": "storage_policy", "length": 100}
...
def retrieveFlowServiceID(service_name, variables):
    ...
    try:
        api_response = service_groups_api.list_service_groups(
            _page=page, _limit=limit, _filter="name eq '" + service_name + "'"
        )
...
def retrieveSecurityPolicyInfo(policy_name, variables):
    ...
    client = ntnx_microseg_py_client.ApiClient(configuration=sdkConfig)
    network_security_policies_api = ntnx_microseg_py_client.NetworkSecurityPoliciesApi(
        api_client=client
    )
...
def retrieveProtectionPolicyInfo(policy_name, variables):
    ...
    payload = {"kind": "protection_rule", "length": 100}
    ...
    for policy in response_data["entities"]:
        if policy["status"]["name"] == policy_name:
            ...
            url = "https://%s:9440/api/nutanix/v3/protection_rules/%s" % (
                variables["PC"],
                poluuid,
            )
...
def retrieveApprovalPolicyInfo(policy_name, variables):
    ...
    url = (
        "https://%s:9440/api/security/v4.0.a1/management/approval-policies"
        % variables["PC"]
    )
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    payload = {}
...
def retrieveReportInfo(reportName, variables):
    ...
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    payload = {"kind": "report_config", "length": 100}
...
def getRunwayForCluster(variables):
    ...
    response = requests.post(
        ...
        headers=headers,
        verify=False,
        auth=(variables["OldPCUsername"], variables["OldPCPassword"]),
    )
    response_data = json.loads(response.text)

    json_expr = parse(
        "$.group_results[0].entity_results[0].data[?(@.name=='capacity.runway')].values[*].values"
    )
...
def retrievePlaybookInfo(name, variables):
    ...
    json_expr = parse(
        "$.group_results[*].entity_results[?(@.data[0].values[0].values[0]=='"
        + name
        + "')].entity_id"
    )
    ...
    if playbookID is None:
        return False, None
    else:
        url = "https://%s:9440/api/nutanix/v3/action_rules/%s" % (
            variables["PC"],
            playbookID,
        )
...
# = retrieveVpcId
# ========================================================================
def retrieveVpcId(name, variables):
    sdkConfig = confSDKClient(
        variables["PC"], variables["PCUser"], variables["PCPassword"]
    )
    client = ntnx_networking_py_client.ApiClient(configuration=sdkConfig)
...
def getBpContent(bpName, variables):
    ...
    headers = {"Content-Type": "application/json", "Accept": "application/json"}
    payload = {"kind": "blueprint", "filter": "name==" + bpName}
    ...
    jsonpath_expr = parse(
        "$.status.resources.service_definition_list[*].action_list[?(@.name=='action_create')].runbook.task_definition_list"
    )
